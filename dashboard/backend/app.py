...
with app.app_context():
    ...
    # --- Auto-migrate: add new columns / tables to existing SQLite databases ---
    # This block is SQLite-specific legacy migration scaffolding.  When running
    # against Postgres, Alembic owns schema migrations (Step 2).  These pragmas
    # and executescript() calls are not portable to PG and must remain guarded.
    if not _is_pg_backend:
